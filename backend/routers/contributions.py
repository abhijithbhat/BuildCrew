...
async def confirm_contribution(
    ...
    """Peer-confirm a contribution. Updates verification status to 'confirmed'.

    Only a teammate or project lead other than the contributor can confirm it.
    """
    user_id = _get_user_id(current_user)
    ...
    try:
        ...
        # 2. Block self-confirmation
        contributor_id = contribution.get("contributor")
        if contributor_id == user_id:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="You cannot confirm your own contribution. Only a teammate can peer-confirm it.",
            )
        ...
        display_name = (
            ...
            or updated_data.get("contributor_name")
            or f"User {contributor_id[:8]}"
        )
        updated_data["contributor_name"] = display_name
        updated_data["contributor_profile"] = prof_data or None
        ...
        if _is_dev_fallback_error(err_msg):
            ...
            now_iso = datetime.now(timezone.utc).isoformat()
            contribution["verification_status"] = "confirmed"
...
async def dispute_contribution(
    ...
    """Dispute a contribution. Sets verification status to 'needs-review' and visibility to 'private'.

    Only a teammate or project lead other than the contributor can dispute it.
    """
    user_id = _get_user_id(current_user)
    ...
    try:
        ...
        # 2. Block self-dispute
        contributor_id = contribution.get("contributor")
        if contributor_id == user_id:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="You cannot dispute your own contribution.",
            )
        ...
        # 4. Update status to 'needs-review' and visibility to 'private'
        now_iso = datetime.now(timezone.utc).isoformat()
        update_payload = {
