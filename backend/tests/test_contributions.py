...
# ==============================================================================
# 9. Peer Confirmation & Dispute Endpoint Tests (Phase 11)
# ==============================================================================

def test_confirm_contribution_success_by_teammate():
    """Teammate can successfully confirm another member's contribution."""
    mock_teammate = MagicMock(id="user-teammate-bob", email="bob@buildcrew.io")
    app.dependency_overrides[get_current_user] = lambda: mock_teammate

    DEV_PROJECTS_DB["proj-peer-1"] = {
        "id": "proj-peer-1",
        "name": "Peer Verified App",
        "created_by": "user-author-alice",
    }
    DEV_PROJECT_MEMBERS_DB.append({
    ...
    response = client.post("/contributions/c-alice-design/confirm")
    assert response.status_code == 200
    data = response.json()
    assert data["id"] == "c-alice-design"
    assert data["verification_status"] == "confirmed"
    assert data["confirmed_by"] == "user-teammate-bob"


def test_confirm_contribution_blocks_author_self_confirmation():
    """Author cannot self-confirm their own contribution (HTTP 403 Forbidden)."""
    mock_author = MagicMock(id="user-author-alice", email="alice@buildcrew.io")
    app.dependency_overrides[get_current_user] = lambda: mock_author
    ...
    response = client.post("/contributions/c-alice-design/confirm")
    assert response.status_code == 403
    assert "cannot confirm your own contribution" in response.json()["detail"].lower()


def test_confirm_contribution_blocks_non_project_member():
    """Users outside the project cannot confirm contributions (HTTP 403 Forbidden)."""
    mock_outsider = MagicMock(id="user-stranger", email="stranger@random.io")
    app.dependency_overrides[get_current_user] = lambda: mock_outsider
    ...
    response = client.post("/contributions/c-alice-design/confirm")
    assert response.status_code == 403
    assert "must be a member of this project" in response.json()["detail"].lower()


def test_confirm_contribution_not_found():
    mock_user = MagicMock(id="user-teammate-bob", email="bob@buildcrew.io")
    app.dependency_overrides[get_current_user] = lambda: mock_user

    response = client.post("/contributions/non-existent-contrib/confirm")
    assert response.status_code == 404


def test_dispute_contribution_success_by_teammate():
    """Teammate can dispute an inaccurate contribution, setting status to needs-review and visibility to private."""
    mock_teammate = MagicMock(id="user-teammate-bob", email="bob@buildcrew.io")
    app.dependency_overrides[get_current_user] = lambda: mock_teammate
    ...
    response = client.post("/contributions/c-alice-claim/dispute")
    assert response.status_code == 200
    data = response.json()
    assert data["id"] == "c-alice-claim"
    assert data["verification_status"] == "needs-review"
    assert data["dispute_state"] == "disputed"
    assert data["visibility"] == "private"


def test_dispute_contribution_blocks_author_self_dispute():
    """Author cannot self-dispute their own contribution (HTTP 403 Forbidden)."""
    mock_author = MagicMock(id="user-author-alice", email="alice@buildcrew.io")
    app.dependency_overrides[get_current_user] = lambda: mock_author
    ...
    response = client.post("/contributions/c-alice-claim/dispute")
    assert response.status_code == 403
    assert "cannot dispute your own contribution" in response.json()["detail"].lower()


def test_list_project_contributions_counts_self_declared_and_confirmed():
    """Verify that manually logged 'self-declared' contributions are properly counted in draft_count and confirmed_count."""
    mock_lead = MagicMock(id="user-team-lead", email="lead@buildcrew.io")
    app.dependency_overrides[get_current_user] = lambda: mock_lead

    DEV_PROJECTS_DB["proj-count-test"] = {
        "id": "proj-count-test",
        "name": "Accounting Project",
        "created_by": "user-team-lead",
    }
    DEV_PROJECT_MEMBERS_DB.append({
        "project_id": "proj-count-test",
        "user_id": "user-teammate-charlie",
    })

    # Add 1 self-declared contribution from Charlie
    DEV_CONTRIBUTIONS_DB.append({
        "id": "c-charlie-1",
        "project": "proj-count-test",
        "contributor": "user-teammate-charlie",
    ...
    # Step 1: Query contributions -> draft_count must include self-declared!
    res1 = client.get("/projects/proj-count-test/contributions")
    assert res1.status_code == 200
    data1 = res1.json()
    assert data1["total_count"] == 1
    assert data1["draft_count"] == 1
    assert data1["confirmed_count"] == 0

    # Step 2: Team lead confirms Charlie's contribution
    res_confirm = client.post("/contributions/c-charlie-1/confirm")
    assert res_confirm.status_code == 200
    assert res_confirm.json()["verification_status"] == "confirmed"

    # Step 3: Query contributions again -> confirmed_count must be 1, draft_count 0!
    res2 = client.get("/projects/proj-count-test/contributions")
    assert res2.status_code == 200
    data2 = res2.json()
    assert data2["total_count"] == 1
    assert data2["draft_count"] == 0
